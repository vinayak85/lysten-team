...
def get_lat_long_details(emp, date):
  ...
  FROM(  
SELECT name,latitude,longitude,SUBSTRING(creation, 12, 5) as time_call,(SELECT IF(objective != '' && doctor_flag=0 && meeting_flag=0 && camp_flag=0 && leave_flag=0,
...
where select_date = {0} and user={1}) as subject,"" as patch_id,join_name1 as "jwf_with",join_name2  as "jwf_with2"
  FROM 1bd3e0294da19198.`tabObjective`where select_date={0} and user ={1}) as temp 
  ...
  FROM 1bd3e0294da19198.`tabChemist Call`where date={0} and user_id ={1}
