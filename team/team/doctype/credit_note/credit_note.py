...
from erpnext.accounts.doctype.credit_note.pos import update_multi_mode_option
...
class credit_note(SellingController):
	def __init__(self, arg1, arg2=None):
		super(credit_note, self).__init__(arg1, arg2)
		self.status_updater = [{
	...
	def validate(self):
		super(credit_note, self).validate()
	...
	def set_missing_values(self, for_validate=False):
		...
		super(credit_note, self).set_missing_values(for_validate)

		if pos:
			return {"print_format": pos.get("print_format") }

	def update_time_sheet(self, credit_note):
	...
	def update_time_sheet_detail(self, timesheet, args, credit_note):
		for data in timesheet.time_logs:
			if (self.project and args.timesheet_detail == data.name) or \
				(not self.project and not data.credit_note) or \
				(not credit_note and data.credit_note == self.name):
	...
	def validate_with_previous_doc(self):
		super(credit_note, self).validate_with_previous_doc({
			"Sales Order": {
	...
	def validate_warehouse(self):
		super(credit_note, self).validate_warehouse()
	...
	def make_item_gl_entries(self, gl_entries):
		...
		if cint(self.update_stock) and \
			...
			gl_entries += super(credit_note, self).get_gl_entries()
	...
	def validate_serial_against_sales_invoice(self):
		...
		for item in self.items:
			...
			for serial_no in item.serial_no.split("\n"):
				credit_note = frappe.db.get_value("Serial No", serial_no, "sales_invoice")
				if credit_note and self.name != credit_note:
					frappe.throw(_("Serial Number: {0} is already referenced in Sales Invoice: {1}".format(
						serial_no, credit_note
					)))
...
def make_delivery_note(source_name, target_doc=None):
	...
	doclist = get_mapped_doc("credit_note", source_name, 	{
		"credit_note": {
...
def make_sales_return(source_name, target_doc=None):
	...
	return make_return_doc("credit_note", source_name, target_doc)
