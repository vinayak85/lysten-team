...
@frappe.whitelist()
def get_pos_data():
	doc = frappe.new_doc('credit_note')
	doc.is_pos = 1;
...
def get_meta():
	doctype_meta = {
		...
		'invoice': frappe.get_meta('credit_note')
	}

	for row in frappe.get_all('DocField', fields = ['fieldname', 'options'],
		filters = {'parent': 'credit_note', 'fieldtype': 'Table'}):
...
def make_email_queue(email_queue):
	...
	for key, data in email_queue.items():
		name = frappe.db.get_value('credit_note', {'offline_pos_name': key}, 'name')
		data = json.loads(data)
		sender = frappe.session.user
		print_format = "POS Invoice"
		attachments = [frappe.attach_print('credit_note', name, print_format= print_format)]

		make(subject = data.get('subject'), content = data.get('content'), recipients = data.get('recipients'),
			sender=sender,attachments = attachments, send_email=True,
			doctype='credit_note', name=name)
...
def save_invoice(e, si_doc, name):
	if not frappe.db.exists('credit_note', {'offline_pos_name': name}):
		si_doc.docstatus = 0
