...
def check_duplicate(year, month,stockist):
	...
	c = frappe.db.sql("""SELECT ifnull(count(name),0) as name FROM tabSecondary where name like {0} and  docstatus !=2""".format("'"+doc_name+"'"), as_dict=1);
	#return c[0].name;
	dict = {'count': 0}
	dict['count'] = c[0].name;
	return dict;
...
max(tbl.cr_avg_rate) as cr_avg_rate,
...
sum(t_sit.base_net_amount) as q_amt,(sum(t_sit.free_quantity)*max(t_sit.base_net_rate)) as f_amt,
max(t_sit.base_net_rate) as avg_rate,
0 as cr_qty, 0 as cr_f_qty,0 as cr_tot_qty,
0 as cr_q_amt,0 as cr_f_amt,
max(t_sit.base_net_rate) as cr_avg_rate,
0 as close_tot,
0 as close_qty,
0 as close_free
from tabItem as t_i
...
sum(t_sit.base_net_amount) as cr_q_amt,(sum(t_sit.free_quantity)*max(t_sit.base_net_rate)) as cr_f_amt,
max(t_sit.base_net_rate) as cr_avg_rate,
0 as close_tot,
0 as close_qty,
0 as close_free
...
class Secondary(Document):
	...
	if(duplicate_pos>=0):
		...
		var aa=', '.join("%s: %s" % item for item in self.__dict__.keys());
		frappe.msgprint(_(aa));
